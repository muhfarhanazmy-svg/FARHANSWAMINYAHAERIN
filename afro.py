...
def booking():
    ...
    if bentrok:
        nama_ruang = next(g["nama"] for g in gedung if g["id"] == id_g)
        print(f"\n  [!] Jadwal bentrok!")
...
def validasi_tanggal(tgl):
    ...
    if not re.fullmatch(r"\d{2}/\d{2}", tgl):
        return False, "Format tanggal harus dd/mm (contoh: 25/06)"
    dd, mm = int(tgl[:2]), int(tgl[3:])
    if not (1 <= mm <= 12):
        return False, "Bulan harus antara 01 sampai 12"
    if not (1 <= dd <= 31):
        return False, "Tanggal harus antara 01 sampai 31"
    return True, ""
