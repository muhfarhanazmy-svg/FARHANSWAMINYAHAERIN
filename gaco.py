...
Library:
    ...
    - gtts      : Google Text-to-Speech untuk audio pemanggilan
    - collections.deque : implementasi queue yang efisien
"""
...
def buat_audio(teks: str) -> str:
    ...
    lalu mengembalikannya sebagai string base64 agar bisa
    diputar langsung di browser via HTML audio tag.
    """
...
# --- Penjelasan Konsep ---
with st.expander("📚 Apa itu Queue & FIFO?", expanded=False):
    st.markdown("""
    **Queue (Antrian)** adalah struktur data linear yang mengikuti prinsip **FIFO**:
    > *First In, First Out — yang pertama masuk adalah yang pertama dilayani.*

    | Operasi    | Deskripsi                              | Posisi       |
    |------------|----------------------------------------|--------------|
    | `enqueue`  | Menambahkan elemen ke antrian          | Belakang (rear) |
    | `dequeue`  | Mengambil & menghapus elemen           | Depan (front)   |
    | `front`    | Melihat elemen paling depan            | Depan        |
    | `rear`     | Melihat elemen paling belakang         | Belakang     |

    **Analogi:** Bayangkan antrian kasir — orang yang datang pertama akan dilayani pertama.
    """)
